...
const swapMNEoMNE = async (amount: number) => {
  console.log('Mock swapMNEoMNE:', amount);
...
- Users unable to convert USD to MNE for withdrawal
...
- `POST /api/user/:telegramId/swap/USD-to-MNE` - Convert USD to MNE
- `POST /api/user/:telegramId/swap/MNE-to-USD` - Convert MNE to USD
...
**Impact**:
- Admin unable to control USD-MNE conversion rate
...
client/src/components/SwapInterface.tsx:
- // Mock functions
+ import { useSwapMNEoMNE, useSwapMNEToUSD, useExchangeRate }
...
```bash
1. Purchase a new slot with 10 USD
...
2. Go to main page
3. Enter swap amount (e.g., 5 USD)
4. Click "Swap to MNE"
5. Should see: API call to /api/user/.../swap/USD-to-MNE
6. Should see success alert with conversion details
7. Check Activity Log - should have SWAP_USD_TO_MNE entry
...
2. Should see "Currency Swap (USD ⇄ MNE)" section
3. Shows current exchange rate
4. Can select direction (USD→MNE or MNE→USD)
...
describe('Currency Swap', () => {
  it('should swap USD to MNE with correct rate', async () => {
    const response = await swapMNEoMNE({ telegramId, amount: 10 });
    expect(response.USDAmount).toBe(-10);
    expect(response.MNEAmount).toBeGreaterThan(0);
    expect(response.rate).toBeGreaterThan(0);
  });

  it('should create swap transaction record', async () => {
    await swapMNEoMNE({ telegramId, amount: 10 });
...
describe('Admin Exchange Rate', () => {
  ...
  it('should apply rate to user swaps', async () => {
    ...
    const swap = await swapMNEoMNE({ telegramId, amount: 10 });
