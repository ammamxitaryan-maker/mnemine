...
export const useSwap = (telegramId: string) => {
  ...
  const swapCfmToCfmt = async (amount: number) => {
    setIsLoading(true);
    try {
      const response = await apiClient.post(`/user/${telegramId}/swap/cfm-to-cfmt`, { amount });
      return response;
    } catch (error) {
      console.error('Error swapping CFM to CFMT:', error);
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const swapCfmtToCfm = async (amount: number) => {
    setIsLoading(true);
    try {
      const response = await apiClient.post(`/user/${telegramId}/swap/cfmt-to-cfm`, { amount });
      return response;
    } catch (error) {
      console.error('Error swapping CFMT to CFM:', error);
      throw error;
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="real-time-income">
      ...
      <div className="income-display">
        <div className="current-income">
          <span>Current: {income.totalCurrentIncome.toFixed(4)} CFM</span>
        </div>
        <div className="projected-income">
          <span>Projected: {income.totalProjectedIncome.toFixed(4)} CFM</span>
        </div>
      </div>
        ...
        {income.slots.map(slot => (
          <div key={slot.id} className={`slot ${slot.isLocked ? 'locked' : ''}`}>
            <div className="slot-info">
              ...
              <span>Principal: {slot.principal} CFM</span>
              <span>Rate: {slot.rate}%</span>
            </div>
            <div className="slot-income">
              <span>Current: {slot.currentIncome.toFixed(4)} CFM</span>
              <span>Projected: {slot.projectedIncome.toFixed(4)} CFM</span>
            </div>
            {slot.isLocked && (
...
export const CurrencySwap: React.FC<CurrencySwapProps> = ({ telegramId }) => {
  ...
  const [swapDirection, setSwapDirection] = useState<'cfm-to-cfmt' | 'cfmt-to-cfm'>('cfm-to-cfmt');
  
  const { swapCfmToCfmt, swapCfmtToCfm, isLoading } = useSwap(telegramId);
  ...
  const handleSwap = async () => {
    ...
    try {
      if (swapDirection === 'cfm-to-cfmt') {
        await swapCfmToCfmt(swapAmount);
      } else {
        await swapCfmtToCfm(swapAmount);
      }
  ...
  return (
    <div className="currency-swap">
      ...
      <div className="swap-controls">
        ...
        >
          <option value="cfm-to-cfmt">CFM → CFMT</option>
          <option value="cfmt-to-cfm">CFMT → CFM</option>
        </select>
    ...
    <div className="dashboard">
      <header>
        ...
        <div className="user-balance">
          <span>CFM Balance: {userData.user.wallets.find(w => w.currency === 'CFM')?.balance.toFixed(4)}</span>
          <span>CFMT Balance: {userData.user.wallets.find(w => w.currency === 'CFMT')?.balance.toFixed(4)}</span>
        </div>
      </header>
        ...
        {slots.map(slot => (
          <div key={slot.id} className={`slot ${slot.isLocked ? 'locked' : ''}`}>
            <div className="slot-info">
              ...
              <p>Type: {slot.type}</p>
              <p>Principal: {slot.principal} CFM</p>
