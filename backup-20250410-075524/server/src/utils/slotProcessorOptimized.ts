...
const processUserSlots = async (userId: string, slots: any[]) => {
  const now = new Date();
  const cfmWallet = slots[0].user.wallets.find((w: Wallet) => w.currency === 'CFM');
  
  if (!cfmWallet) {
    throw new Error(`CFM wallet not found for user ${userId}`);
  }
  ...
  for (const slot of slots) {
    ...
    activityLogs.push({
      ...
      amount: earnings,
      description: `Automatic slot closure - earned ${earnings.toFixed(4)} CFM from ${slot.principal} CFM investment`
    });
  }
