...
export const buyNewSlot = async (req: Request, res: Response) => {
  ...
  if (!amount || typeof amount !== 'number' || amount < MINIMUM_SLOT_INVESTMENT) {
    return res.status(400).json({ error: `Minimum investment is ${MINIMUM_SLOT_INVESTMENT} CFM` });
  }
...
const processSlotBatch = async (slots: any[], now: Date) => {
  ...
  for (const slot of slots) {
    try {
      ...
      const cfmWallet = slot.user.wallets.find((w: Wallet) => w.currency === 'CFM');
      if (cfmWallet) {
        await prisma.$transaction([
          prisma.wallet.update({
            where: { id: cfmWallet.id },
          ...
          prisma.activityLog.create({
            data: {
              ...
              description: `Automatic slot closure - earned ${finalEarnings.toFixed(4)} CFM from ${slot.principal} CFM investment`,
            },
          }),
        ]);
