...
export const processTodayPayouts = async (req: Request, res: Response) => {
  try {
    ...
    for (const investment of expiringInvestments) {
      ...
      const cfmWallet = await prisma.wallet.findFirst({
        where: {
          userId: investment.userId,
          currency: 'CFM'
        }
      ...
      await prisma.activityLog.create({
        data: {
          ...
          amount: totalPayout,
          description: `Investment completed: ${investment.amount} CFM + ${expectedReturn} CFM earnings`
        }
      });
    }
    ...
    res.status(200).json({ 
      success: true,
      message: `Processed ${expiringInvestments.length} payouts totaling ${totalAmount.toFixed(4)} CFM`,
      data: {
        payoutId: dailyPayout.id,
        totalAmount: totalAmount,
        totalUsers: expiringInvestments.length,
