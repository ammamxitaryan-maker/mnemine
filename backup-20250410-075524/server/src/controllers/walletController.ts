...
export const withdrawFunds = async (req: Request, res: Response) => {
  ...
  // Проверяем, разрешен ли прямой вывод CFM
  if (DIRECT_CFM_WITHDRAWAL_DISABLED) {
    return res.status(400).json({ 
      error: 'Direct CFM withdrawal is disabled. Please convert to CFMT first using the swap function.' 
    });
  }
  ...
  try {
    ...
    if (!hasMinimumDeposit && activeReferralsCount < 3) {
      return res.status(400).json({ 
        error: `You need either a minimum deposit of ${MINIMUM_DEPOSIT_FOR_WITHDRAWAL} CFM or 3 active referrals to withdraw.` 
      });
    }
