...
process.on('unhandledRejection', (reason, promise) => {
  ...
  console.error('[SERVER] Promise rejection stack:', reason instanceof Error ? reason.stack : 'No stack trace');
  // Don't exit immediately in production, allow graceful shutdown
  if (process.env.NODE_ENV === 'production') {
  ...
  try {
    ...
    if (user.telegramId === ADMIN_TELEGRAM_ID) {
      const wallet = await prisma.wallet.findFirst({
        where: { userId: user.id, currency: 'CFM' },
      });
      ...
      if (wallet) {
        if (wallet.balance < 50000) {
          ...
          console.log(`[SEED] Updated admin balance to 50000 CFM`);
        }
      } else {
        ...
        console.log(`[SEED] Created admin wallet with 50000 CFM`);
      }
    }
  } catch (error) {
