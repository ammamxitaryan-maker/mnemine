      ...
      user = await prisma.$transaction(async (tx) => {
        const newUser = await tx.user.create({
          data: {
            ...
            wallets: { create: { currency: 'CFM', balance: 0 } }, // Начинаем с 0, 3 CFM автоматически инвестируются
            miningSlots: { 
              create: { 
                principal: AUTO_INVEST_WELCOME_AMOUNT, // 3 CFM автоматически инвестируется
                startAt: new Date(), 
