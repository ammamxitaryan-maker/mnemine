...
- **Gold for CFM**: `#fbbf24` - Luxury, value
...
- ✅ Direction selector (CFM→CFMT / CFMT→CFM)
...
const claim = () => {
  if (userData && userData.balance < 3) {
    showError('❌ Your balance must be at least 3 CFM to claim earnings.');
    return;
  }
  originalClaim();
};
...
```tsx
// Gold text for CFM amounts
<span className="text-gold font-mono">10.50 CFM</span>
...
```
1. Have balance < 3 CFM
2. Try to claim
3. Should see toast: "Your balance must be at least 3 CFM..."
4. Add balance > 3 CFM
5. Claim should work
```
...
**Gold** - CFM amounts, money, value
```tsx
<span className="text-gold">{amount.toFixed(2)} CFM</span>
```
