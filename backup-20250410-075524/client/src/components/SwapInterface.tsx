...
export const SwapInterface: React.FC<SwapInterfaceProps> = ({ telegramId }) => {
  ...
  const handleSwap = useCallback(async () => {
    ...
    try {
      ...
      if (swapDirection === 'cfm-to-cfmt') {
        ...
        setLastSwap(result);
        alert(`✅ Swap Successful!\n\n${swapAmount.toFixed(4)} CFM → ${result.cfmtAmount.toFixed(4)} CFMT\nRate: ${result.rate.toFixed(4)}`);
      } else {
        const result = await swapCfmtToCfmMutation.mutateAsync(swapAmount);
        setLastSwap(result);
        alert(`✅ Swap Successful!\n\n${swapAmount.toFixed(4)} CFMT → ${result.cfmAmount.toFixed(4)} CFM\nRate: ${result.rate.toFixed(4)}`);
      }
  ...
  }, [amount, swapDirection, swapCfmToCfmtMutation, swapCfmtToCfmMutation]);
  ...
  return (
    <div className="swap-interface">
      <div className="swap-interface__header">
        <h3 className="swap-interface__title">Currency Swap (CFM ⇄ CFMT)</h3>
        {rateLoading ? (
      ...
      <div className="swap-interface__form">
        ...
        <div className="swap-interface__amount">
          ...
          <div className="swap-interface__amount-input-group">
            ...
            <span className="swap-interface__amount-currency">
              {swapDirection === 'cfm-to-cfmt' ? 'CFM' : 'CFMT'}
            </span>
          </div>
        </div>
        ...
        {amount && rateData && (
          <div className="swap-interface__preview">
            ...
            <div className="swap-interface__preview-amount">
              {swapDirection === 'cfm-to-cfmt' 
                ? `${(parseFloat(amount) * rateData.rate).toFixed(4)} CFMT`
                : `${(parseFloat(amount) / rateData.rate).toFixed(4)} CFM`
              }
            </div>
          </div>
        )}
        ...
        >
          {swapDirection === 'cfm-to-cfmt' ? 'Swap to CFMT' : 'Swap to CFM'}
        </LoadingButton>
      </div>
      ...
      {lastSwap && (
        <div className="swap-interface__result">
          ...
          <div className="swap-interface__result-details">
            <div>CFM: {lastSwap.cfmAmount?.toFixed(4) || '0.0000'}</div>
            <div>CFMT: {lastSwap.cfmtAmount?.toFixed(4) || '0.0000'}</div>
