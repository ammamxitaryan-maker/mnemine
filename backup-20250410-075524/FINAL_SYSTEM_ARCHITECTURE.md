...
2. **Сложная система ограничений** - первый вывод 3 CFM, нужны 3 реферала + завершенный период
...
5. **Две валюты с плавающим курсом** - CFM ↔ CFMT с административным управлением
...
- swapController.ts         // Конвертация CFM ↔ CFMT
...
const weeklyRate = amount >= 100 ? 0.35 : 0.30; // 35% для 100+ CFM, 30% для остальных
```

### **2. СИСТЕМА ОГРАНИЧЕНИЙ**
```typescript
// Первый вывод только 3 CFM, нужны 3 реферала
...
const maxReferralIncome = user.cfmBalance;
...
const processDailyPayouts = async () => {
  ...
  for (const investment of expiringInvestments) {
    ...
    await prisma.wallet.update({
      where: { userId: investment.userId, currency: 'CFM' },
...
✅ **Автоматическим инвестированием** - 3 CFM приветственный бонус  
✅ **Системой ограничений** - первый вывод 3 CFM, нужны 3 реферала  
✅ **Двухуровневой доходностью** - 30% до 100 CFM, 35% для 100+ CFM  
