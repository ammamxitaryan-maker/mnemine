...
// Конвертировать CFM в CFMT
POST /user/:telegramId/swap/cfm-to-cfmt
Body: { amount: number }
Response: {
  message: string,
  cfmAmount: number,
...
// Конвертировать CFMT в CFM
POST /user/:telegramId/swap/cfmt-to-cfm
Body: { amount: number }
Response: {
  message: string,
  cfmAmount: number,
...
export const processExpiredSlots = async () => {
  try {
    ...
    for (const slot of expiredSlots) {
      ...
      // Обновление баланса
      const cfmWallet = slot.user.wallets.find(w => w.currency === 'CFM');
      if (cfmWallet) {
        await prisma.$transaction([
          ...
          prisma.activityLog.create({
            data: {
              ...
              description: `Automatic slot closure - earned ${finalEarnings.toFixed(4)} CFM`
            }
          })
        ]);
