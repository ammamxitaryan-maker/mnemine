...
import { REFERRAL_SIGNUP_BONUS, BASE_STANDARD_SLOT_WEEKLY_RATE, WELCOME_BONUS_AMOUNT } from '../constants';
      ...
      user = await prisma.$transaction(async (tx) => {
        const newUser = await tx.user.create({
          data: {
            ...
            miningSlots: { create: { principal: 1.00, startAt: new Date(), lastAccruedAt: new Date(), effectiveWeeklyRate: BASE_STANDARD_SLOT_WEEKLY_RATE, expiresAt: new Date(Date.now() + 7 * 24 * 3600 * 1000), isActive: true } },
