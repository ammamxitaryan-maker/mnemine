...
import { depositFunds, withdrawFunds, claimEarnings } from '../walletControllerImproved';
  ...
  describe('withdrawFunds', () => {
    ...
    it('should reject withdrawal below minimum balance requirement', async () => {
      const mockUser = {
        id: 'user1',
        telegramId: '123456789',
        wallets: [{ id: 'wallet1', currency: 'CFM', balance: 10 }],
        miningSlots: [],
      };
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        success: false,
        error: 'Minimum balance for withdrawal is 20.00 CFM'
      });
    });
  });
