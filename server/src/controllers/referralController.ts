...
import { REFERRAL_3_IN_3_DAYS_BONUS, RANK_REFERRAL_BONUS_PERCENTAGE } from '../constants';
...
export const claimReferralStreakBonus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!userIsEligible) {
      const userCurrentBalance = user.wallets.find((w: Wallet) => w.currency === 'CFM')?.balance || 0;
