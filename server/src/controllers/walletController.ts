...
import { REFERRAL_COMMISSIONS, RESERVE_FUND_PERCENTAGE, MINIMUM_WITHDRAWAL_REGULAR, WITHDRAWAL_FEE_PERCENTAGE, WITHDRAWAL_REFERRAL_REQUIREMENT, WITHDRAWAL_SLOT_REQUIREMENT, REFERRAL_DEPOSIT_BONUS, MINIMUM_WITHDRAWAL_FIRST_100, FIRST_100_WITHDRAWALS_LIMIT, WITHDRAWAL_DAILY_LIMIT, WITHDRAWAL_MIN_BALANCE_REQUIREMENT, REFERRAL_INCOME_CAP_THRESHOLD, REFERRAL_ZERO_IN_7_DAYS_PENALTY_ENABLED, RANKED_REFERRAL_COMMISSIONS_L1, RANKED_REFERRAL_COMMISSIONS_L2, RANKED_REFERRAL_COMMISSIONS_L3 } from '../constants'; // Updated import for ranked commissions
...
import { validateAmount, validateAddress, sanitizeInput } from '../utils/validation';
...
export const depositFunds = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (!depositorWallet) throw new Error('Depositor CFM wallet not found');
...
export const withdrawFunds = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!cfmWallet) return res.status(400).json({ error: 'CFM wallet not found' });

    if (cfmWallet.balance < WITHDRAWAL_MIN_BALANCE_REQUIREMENT) {
      return res.status(400).json({ error: `Minimum balance for withdrawal is ${WITHDRAWAL_MIN_BALANCE_REQUIREMENT.toFixed(2)} CFM` });
    }
    ...
    if (amount < minimumWithdrawal) return res.status(400).json({ error: `Minimum withdrawal is ${minimumWithdrawal.toFixed(2)} CFM` });

    if (cfmWallet.balance < amount) return res.status(400).json({ error: 'Insufficient balance' });
