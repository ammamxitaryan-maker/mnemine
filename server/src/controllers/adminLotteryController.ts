...
export const getLotteryParticipants = async (req: Request, res: Response) => {
  try {
    ...
    const participantsWithStats = participants.map(participant => ({
      ...
      cfmBalance: participant.wallets[0]?.balance || 0,
...
export const selectLotteryWinner = async (req: Request, res: Response) => {
  ...
  try {
    const ticket = await prisma.lotteryTicket.findUnique({
      ...
      include: {
        user: {
          include: { wallets: { where: { currency: 'CFM' } } }
        },
        lottery: true
      }
    });
    ...
    const cfmWallet = ticket.user.wallets[0];
    if (!cfmWallet) {
      return res.status(400).json({ error: 'User CFM wallet not found' });
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.activityLog.create({
        data: {
          ...
          amount: prizeAmount,
          description: `Admin-selected lottery winner - Prize: ${prizeAmount.toFixed(4)} CFM`,
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.activityLog.create({
        data: {
          ...
          description: `Admin removed lottery win - Prize removed: ${(ticket.prizeAmount || 0).toFixed(4)} CFM`,
          sourceUserId: adminUserId,
        },
      });
    });
...
export const getLotteryStats = async (req: Request, res: Response) => {
  try {
    ...
    const totalRevenue = totalTickets * 1.0; // 1 CFM per ticket
