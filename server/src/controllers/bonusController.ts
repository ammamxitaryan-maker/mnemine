...
import { DAILY_BONUS_AMOUNT, LEADERBOARD_BONUS_AMOUNT, INVESTMENT_GROWTH_BONUS_AMOUNT, DIVIDEND_BASE_RATE, DIVIDEND_RAND_MIN, DIVIDEND_RAND_MAX, DIVIDEND_COOLDOWN_HOURS } from '../constants'; // Added dividend constants
import { ActivityLogType, Wallet } from '@prisma/client';
import { isUserEligible } from '../utils/helpers';
import { userSelectWithoutMiningSlots } from '../utils/dbSelects'; // Import userSelect
...
export const getBonusesSummary = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const hasRecentInvestment = await prisma.activityLog.count({
      where: { userId: user.id, type: { in: [ActivityLogType.DEPOSIT, ActivityLogType.NEW_SLOT_PURCHASE, ActivityLogType.SLOT_EXTENSION, ActivityLogType.BOOSTER_PURCHASE, ActivityLogType.REINVESTMENT] }, createdAt: { gte: sevenDaysAgo } },
    }) > 0;
...
export const claimDailyBonus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: `Claimed ${DAILY_BONUS_AMOUNT} CFM successfully!` });
  } catch (error) {
    ...
    await prisma.$transaction([
      ...
      prisma.activityLog.create({
        data: {
          ...
          description: `Claimed leaderboard top 10 bonus of ${LEADERBOARD_BONUS_AMOUNT} CFM.`,
          ipAddress: ipAddress,
        },
      }),
    ]);

    res.status(200).json({ message: `Claimed ${LEADERBOARD_BONUS_AMOUNT} CFM leaderboard bonus!` });
  } catch (error) {
      ...
      where: {
        ...
        type: {
          in: [
            ...
            ActivityLogType.BOOSTER_PURCHASE,
    ...
    await prisma.$transaction([
      ...
      prisma.activityLog.create({
        data: {
          ...
          description: `Claimed investment growth bonus of ${INVESTMENT_GROWTH_BONUS_AMOUNT} CFM.`,
          ipAddress: ipAddress,
        },
      }),
    ]);

    res.status(200).json({ message: `Claimed ${INVESTMENT_GROWTH_BONUS_AMOUNT} CFM investment growth bonus!` });
  } catch (error) {
