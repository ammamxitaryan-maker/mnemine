...
const getAchievementDescription = (id: string, reward: number) => `Achievement Reward: ${id} (${reward} CFM)`; // Changed to CFM
...
export const getAchievementsStatus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const activityLogs = await prisma.activityLog.findMany({
      where: {
        userId: user.id,
        type: { in: [ActivityLogType.DEPOSIT, ActivityLogType.BOOSTER_PURCHASE, ActivityLogType.TASK_REWARD] }, // Use ActivityLogType
      },
    });
      ...
      Object.values(achievements).map(async (ach) => {
        ...
        switch (ach.id) {
          ...
          case 'FIVE_REFERRALS':
            ...
            break;
          case 'BUY_BOOSTER':
            isCompleted = activityLogs.some(log => log.type === ActivityLogType.BOOSTER_PURCHASE); // Use ActivityLogType
            break;
