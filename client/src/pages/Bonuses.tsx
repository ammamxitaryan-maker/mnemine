...
} from 'shared'; // Updated import path
...
const Bonuses = () => {
  ...
  const bonusCardsConfig = [
    ...
    },
    {
      ...
      title: "Dividends",
      getStatusText: () => dividendsBonus.status?.canClaim ? `Claim your estimated ${dividendsBonus.status.estimatedAmount} CFM!` : `Next claim in:`,
    ...
    },
    {
      ...
      getStatusText: () => leaderboardBonus.status?.canClaim ? `Claim your ${LEADERBOARD_BONUS_AMOUNT} CFM!` : (leaderboardBonus.status?.isInTop10 ? `Next claim in:` : `Reach Top 10 to claim`),
      getButtonText: () => `Claim ${LEADERBOARD_BONUS_AMOUNT} CFM`,
    ...
    },
    {
      ...
      getStatusText: () => investmentGrowthBonus.status?.canClaim ? `Claim your ${INVESTMENT_GROWTH_BONUS_AMOUNT} CFM!` : (investmentGrowthBonus.status?.hasRecentInvestmentActivity ? `Next claim in:` : `Invest more to claim`),
      getButtonText: () => `Claim ${INVESTMENT_GROWTH_BONUS_AMOUNT} CFM`,
    ...
    },
    {
      ...
      getStatusText: () => referralStreakBonus.status?.canClaim ? `Claim your ${REFERRAL_3_IN_3_DAYS_BONUS} CFM!` : `Invite 3 friends in 3 days (${referralStreakBonus.status?.referralCountIn3Days ?? 0}/3)`,
      getButtonText: () => `Claim ${REFERRAL_3_IN_3_DAYS_BONUS} CFM`,
