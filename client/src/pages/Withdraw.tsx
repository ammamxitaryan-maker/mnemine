...
const Withdraw = () => {
  ...
  return (
    <div className="flex flex-col justify-between min-h-screen text-white p-4">
      <div>
        <PageHeader titleKey="withdraw.title" backTo="/wallet" />

        <Card className="bg-gray-900 border-primary mb-6">
          <CardHeader>
            <CardTitle>{t('withdraw.cardTitle')}</CardTitle>
            <CardDescription className="text-gray-400">
              {t('withdraw.currentBalance')} <span className="text-gold font-bold">{currentBalance.toFixed(4)} CFM</span>
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="text-sm text-gray-400 space-y-1">
              <p>{t('withdraw.fee', { feePercentage: feePercentage * 100 })} <span className="font-mono text-destructive">{fee} CFM</span></p>
              <p>{t('withdraw.receiving')} <span className="font-mono text-emerald">{receiving} CFM</span></p>
            </div>
          </CardContent>
          <CardFooter>
            <Button
