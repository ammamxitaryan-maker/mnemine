...
export const BottomNavBar = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 bg-gray-900/80 backdrop-blur-sm border-t border-gray-700 z-50">
      <div className="flex justify-around items-center h-full max-w-md mx-auto">
        {navItems.map(({ to, icon: Icon, labelKey }) => (
          <NavLink key={to} to={to} className={navLinkClasses} end>
            {({ isActive }) => (
              <>
                {isActive && <span className="absolute top-0 h-0.5 w-8 bg-purple-400 rounded-full animate-in fade-in-0 zoom-in-95" />}
