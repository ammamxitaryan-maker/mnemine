  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      setDisplayValue(prev => {
        ...
        const diff = Math.abs(currentProjectedValue - prev);
        return diff > 0.0001 ? currentProjectedValue : prev;
      });

      animationFrameId.current = requestAnimationFrame(animate);
    };

    // Start animation only if baseValue is a valid number and weeklyGrowthRate is positive
    if (typeof baseValue === 'number' && !isNaN(baseValue) && weeklyGrowthRate > 0) {
      animationFrameId.current = requestAnimationFrame(animate);
    }
