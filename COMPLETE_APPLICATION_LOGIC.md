...
### Основные валюты:
- **USD:** Основная валюта приложения
...
2. **Создание пользователя:**
   ...
   - USD кошелек с балансом 0
   - Заблокированный слот на 3 USD
...
3. **Приветственный слот:**
   ```typescript
   {
     principal: 3.0,           // 3 USD автоматически инвестируется
...
4. **Реферальные бонусы:**
   - Если есть реферер → реферер получает 3 USD
   - Если реферер делает первый депозит → +1 USD бонус

### Результат регистрации:
- **Баланс:** 0 USD (все в слоте)
- **Слот:** 3 USD заблокированы на 7 дней
- **Доход через 7 дней:** 3.9 USD (3 + 0.9)
...
// Примеры:
// 10 USD, 30%, 1 день = 10 * 0.3 * (1/7) = 0.43 USD
// 10 USD, 30%, 7 дней = 10 * 0.3 = 3.0 USD
// 100 USD, 35%, 7 дней = 100 * 0.35 = 35.0 USD
```
...
{
  "amount": 10.0  // Минимум 1 USD
}

// Ответ:
{
  "message": "Successfully converted 10.0 USD to 9.5 MNE",
  "USDAmount": -10.0,
  "MNEAmount": 9.5,
  "rate": 0.95
}
...
{
  "amount": 9.5  // Минимум 1 MNE
}

// Ответ:
{
  "message": "Successfully converted 9.5 MNE to 10.0 USD",
  "USDAmount": 10.0,
  "MNEAmount": -9.5,
  "rate": 0.95
}
...
- **Минимальная сумма:** 1 USD или 1 MNE
...
1. **При депозите реферала:**
   ```typescript
   // Пример: реферал депонировал 100 USD
   const l1Commission = 100 * 0.25; // 25 USD рефереру
   const l2Commission = 100 * 0.15; // 15 USD рефереру реферера
   ```
...
1. **При закрытии слота:**
   ```typescript
   {
     ...
     message: 'Your mining slot has been automatically closed. You earned 0.9 USD.',
     isRead: false
   }
   ```
...
#### **Кошелек:**
```
POST /api/user/:telegramId/deposit           - Депозит USD
POST /api/user/:telegramId/withdraw          - Вывод USD (заблокирован)
...
POST /api/user/:telegramId/swap/USD-to-MNE  - USD → MNE
POST /api/user/:telegramId/swap/MNE-to-USD  - MNE → USD
...
1. **Регистрация:** Получает 3 USD в заблокированном слоте
2. **Через 7 дней:** Слот автоматически закрывается, получает 3.9 USD
3. **Инвестирование:** Может купить новые слоты от 3 USD
4. **Конвертация:** Может обменять USD на MNE для вывода
5. **Вывод:** Нужны 3 реферала или депозит 5+ USD

### Сценарий 2: Активный инвестор

1. **Инвестиции:** 100 USD в премиум слот (35% доходность)
2. **Доход:** 35 USD за 7 дней
3. **Рефералы:** Получает 25% от депозитов L1, 15% от L2
4. **Конвертация:** Обменивает часть на MNE для вывода
5. **Лотерея:** Покупает билеты за 1 USD
...
4. **Лотерея:** Устанавливает джекпот 670 USD
...
PREMIUM_SLOT_THRESHOLD = 100.0          // 100 USD
...
// Ограничения
MINIMUM_SLOT_INVESTMENT = 3.0           // 3 USD
MINIMUM_CONVERSION_AMOUNT = 1.0         // 1 USD/MNE
MINIMUM_WITHDRAWAL_MNE = 3.0           // 3 MNE
...
- **Системой конвертации валют** (USD ↔ MNE)
